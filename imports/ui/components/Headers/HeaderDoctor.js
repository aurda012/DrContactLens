    ...
    return (
      <header className="app-header navbar">
        <button className="navbar-toggler mobile-sidebar-toggler hidden-lg-up"
                onClick={HeaderDoctor.mobileSidebarToggle}
                type="button"
        >
          &#9776;
        </button>
        ...
        <ul className="nav navbar-nav hidden-md-down">
          <li className="nav-item">
            <a className="nav-link navbar-toggler sidebar-toggler"
               onClick={HeaderDoctor.sidebarToggle}
               href="#"
            >
              <h5>&#9776;</h5>
            </a>
            ...
            <Dropdown isOpen={this.state.notificationsOpen} toggle={this.toggle}>
              ...
              <DropdownMenu className="dropdown-menu-left" style={{ left: '-18vh' }}>
                ...
                {notifications.map(({ message, type }) => {
                  return (<DropdownItem>{type === 'pending' ? <i className="fa fa-cart-plus" /> : <i className="fa fa-check-square" /> } { message }</DropdownItem>);
                })}
              </DropdownMenu>
            </Dropdown>
          </li>
...
export default composeWithTracker(composer)(HeaderDoctor);
