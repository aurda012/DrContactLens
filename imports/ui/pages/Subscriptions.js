...
class Subscription extends React.Component {
    ...
    if (confirm(`Are you sure? You\'ll have access to Doxie until ${subscription.current_period_end}`)) {
      Meteor.call('cancelSubscription', (error) => {
        if (error) {
          ...
          Bert.alert('Sorry to see you go! Thanks for being a doctor :)', 'success');
        }
      });
    }
  }
  ...
  getSubscription() {
    const { doctor, plans } = this.props;
    return {
      status: doctor.subscription.status,
      plan: _.findWhere(plans, { planId: doctor.subscription.plan }) || { label: 'None' },
      current_period_end: epochToHuman(doctor.subscription.current_period_end),
    };
  }
  ...
  renderChangePlan() {
    ...
    return (<div className="ChangePlan">
      <Plans currentPlan={doctor.subscription.plan} />
      <Button bsStyle="success" onClick={this.handleChangeSubscription}>
        {doctor.subscription.status === 'cancelling' ? 'Resubscribe' : 'Change Plan'}
      </Button>
    </div>);
  }
      ...
      <ListGroup>
        ...
        <ListGroupItem>
          <Row>
            ...
            <Col xs={12} sm={6} md={7}>
              <p><strong>{doctor.card.brand}</strong> ending in <strong>{doctor.card.last4}</strong></p>
            </Col>
            <Col xs={12} sm={3} md={2}>
              <Button
...
const composer = (props, onData) => {
  const subscription = Meteor.subscribe('doctor.subscription');
  if (subscription.ready()) {
    const doctor = Doctors.findOne();
    const plans = PlansCollection.find().fetch();
    onData(null, { doctor, plans });
  }
};
